...
def s7_blastn_xml(qry, base, threads):
    ...
    ##Edited for synthetic
    cmd = (f"blastn -num_threads {threads} "
        f"-task blastn-short -word_size 4 "
        f"-query {qry} "
        f"-db {base} "
        f"-out {result_path} "
        f"-dust no -soft_masking false "
        f"-num_alignments 5 "
        f"-outfmt 5")
...
def get_umi(s_umi, dict_umi):
    ...
    return s_result
######################################################################################
###MODIFIED AS PER WEI SHAO"S MiSeq_Consensus.pl
############

def get_consensus(family, threshold=0.8):
    s_consensus = ""
    for i in range(len(family[0])):
        s_column = [s[i] for s in family]
        res = Counter(s_column)
        top_allele, top_count = res.most_common(1)[0]
        top_freq = top_count / len(family)
...
def remove_insertion(seq_list):
    return [seq.replace('-', '') for seq in seq_list] ##not used function

def s8_make_consensus(sample, min_family_size = 5, max_family_size = 1000000, threshold=0.8):
    mutex.acquire()
    ...
    for umi in families:
        ...
        if len(family) >= min_family_size and len(family) <= max_family_size:
            ...
            if len(lens) == 1:
                # lens are the same
                s_consensus = get_consensus(family, threshold=threshold) ###RD
...
def s9_sierrapy_all():
    ...
    for sample in samples:
        ...
        # After each sample finishes, merge any chunked outputs
        s9_dir = os.path.join(hivdrm_work_dir, s9_prefix)
        chunk_files = sorted(glob.glob(os.path.join(s9_dir, f"{sample}.*.json")))
        merged_file = os.path.join(s9_dir, f"{sample}.json")

        if len(chunk_files) > 0:
            print(f"Merging {len(chunk_files)} SierraPy chunks for {sample}")
            merged_data = []
            for cf in chunk_files:
                with open(cf) as fh:
                    try:
                        ...
                        print(f"⚠️  Skipping corrupt {cf}: {e}")
            with open(merged_file, "w") as out:
                json.dump(merged_data, out, indent=2)

            # optional: clean up chunk files to keep folder tidy
            for cf in chunk_files:
                try:
                    os.remove(cf)
                except:
...
def s10_drlink_all():
    ...
    # write DRM.xlsx in same location
    excel_path = os.path.join(hivdrm_work_dir, "DRM.xlsx")
...
def s11_freq_table(sample):
    ...
    ### PIPELINE breaks if any seq is of different length within a csample consensus file. we skip those seqs before moving ahead####
    ##we pad the seqs if the differenc is just 1-2 bp####
    # --- NEW BLOCK: sanitize sequences before alignment ---
    from Bio import SeqIO, Seq

    # --- Load and sanitize consensus sequences ---
    seqs = list(SeqIO.parse(alignment_path, "fasta"))
    if not seqs:
        print(f"Warning: no sequences found for {sample}, skipping.")
        return
    
    # Determine the most common sequence length (mode)
    lengths = [len(s.seq) for s in seqs]
    mode_len = max(set(lengths), key=lengths.count)
    
    # Filter out obvious outliers (e.g., truncated reads)
    seqs = [s for s in seqs if abs(len(s.seq) - mode_len) <= 2]
    if len(seqs) == 0:
        print(f"Warning: all sequences filtered out for {sample}, skipping.")
        return
    
    # --- Clean up and equalize all sequences ---
    max_len = max(len(s.seq) for s in seqs)
    for s in seqs:
        seq_str = str(s.seq).replace(" ", "").replace("\r", "").replace("\n", "")
        # pad or trim to the common max_len
        if len(seq_str) < max_len:
            seq_str = seq_str + "-" * (max_len - len(seq_str))
        elif len(seq_str) > max_len:
            seq_str = seq_str[:max_len]
        s.seq = Seq.Seq(seq_str)
    
    # --- Always write a clean, uniform file ---
    padded_path = alignment_path + ".padded"
    SeqIO.write(seqs, padded_path, "fasta")
    print(f"Step11: using padded file for {sample}: {padded_path}")
    
    # --- Safe read for downstream analysis ---
    alignment = AlignIO.read(padded_path, "fasta")
...
def s11_freq_table_all():
    ...
    excel_path = os.path.join(hivdrm_work_dir, "freq.xlsx")
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
...
def get_args(description):
    ...
    ##RD
    parser.add_argument("--outdir", required = True, help = "output directory for this sample")
    #RD
...
if __name__ == "__main__":
    ...
    ##RD
    hivdrm_work_dir = args.outdir
    os.makedirs(hivdrm_work_dir, exist_ok=True)
    print(f"Writing output to: {hivdrm_work_dir}")
    ###
